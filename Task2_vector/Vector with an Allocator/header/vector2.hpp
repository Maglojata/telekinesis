...
#define __V_C_R__
#include "vector.h"
// #include "../headerAlloc/alloc.h"
#include <iostream>

    template <typename T, typename Allocator>
    vector<T, Allocator>::vector(size_type length, const_reference val)
        : m_size{length}
        , m_capacity{length}
        , m_arr{nullptr}
    {
    ...
    template <typename T, typename Allocator>
    vector<T, Allocator>::vector(size_type length)
    ...
    template <typename T, typename Allocator>
    void
    vector<T, Allocator>::reserve(size_type count) {
            if (count > m_capacity) {
                pointer mew_memory = new value_type[count];
                for (size_t i = 0; i < m_size; ++i) {
                    new_memory[i] = this -> m_arr[i];
                }
                delete [] this -> m_arr;
                this -> m_arr = new_memory;
    ...
    template <typename T, typename Allocator>
    void
    vector<T, Allocator>::resize(size_type size, const_reference val) {
            if (size < this->m_size) {
                this->m_size = size;
            } else {
    ...
    template <typename T, typename Allocator>
    void
    vector<T, Allocator>::insert(size_type index, const_reference val){
        reserve(m_capacity * 2);
        ++this -> m_size;
    ...
    template <typename T, typename Allocator>
    void
    vector<T, Allocator>::erase(size_type index) {
    ...
    template <typename T, typename Allocator>
    bool
    vector<T, Allocator>::empty() {
        return !this -> m_size;
    }

    template <typename T, typename Allocator>
    typename vector<T, Allocator>::size_type
    vector<T, Allocator>::size(){
        return this -> m_size;
    }
